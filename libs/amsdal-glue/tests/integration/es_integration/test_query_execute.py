...
def _register_default_connection() -> Generator[None, None, None]:
    ...
    test_prefix_2 = f'test-{uuid.uuid4().hex}-'
    ...
    with cleanup_elasticsearch_indices(raw_es, test_prefix):
        # Create connection pool - no constructor args, all args go to connect()
        ...
        # Second connection pool for shippings with different prefix
        with cleanup_elasticsearch_indices(raw_es, test_prefix_2):
            connection_mng.register_connection_pool(
                DefaultConnectionPool(
                    ElasticsearchConnection,
                    [{'host': es_host, 'port': es_port, 'scheme': es_scheme}],  # hosts positional arg for connect()
                    index_prefix=test_prefix_2,  # kwargs for connect()
                    basic_auth=(es_user, es_password),
                    instant_refresh=True,
                ),
