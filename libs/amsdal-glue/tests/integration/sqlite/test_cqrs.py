...
def test_schema_command(cqrs_app: CQRSApplication) -> None:
    ...
    connections = [
        cqrs_app.query_connection_manager.get_connection_pool(user_schema.name).get_connection(),
        cqrs_app.command_connection_manager.get_connection_pool(user_schema.name).get_connection(),
    ]
...
def test_data_command(cqrs_app: CQRSApplication):
    ...
    connections = [
        ...
        cqrs_app.command_connection_manager.get_connection_pool('customers').get_connection(),
    ]
...
def test_data_query(cqrs_app: CQRSApplication, mocker: MockerFixture):
    service = Container.services.get(DataCommandService)
    result = service.execute(
        DataCommand(
            mutations=[
