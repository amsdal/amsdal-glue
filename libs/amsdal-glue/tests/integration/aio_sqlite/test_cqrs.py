...
async def test_schema_command(cqrs_app: AsyncCQRSApplication) -> None:
    ...
    connections = [
        await cqrs_app.query_connection_manager.get_connection_pool(user_schema.name).get_connection(),
        await cqrs_app.command_connection_manager.get_connection_pool(user_schema.name).get_connection(),
    ]
...
@pytest.mark.asyncio
async def test_data_command(cqrs_app: AsyncCQRSApplication) -> None:
    ...
    connections = [
        await cqrs_app.query_connection_manager.get_connection_pool('customers').get_connection(),
        await cqrs_app.command_connection_manager.get_connection_pool('customers').get_connection(),
    ]
...
@pytest.mark.asyncio
async def test_data_query(cqrs_app: AsyncCQRSApplication, mocker: MockerFixture) -> None:
    service = Container.services.get(AsyncDataCommandService)
    result = await service.execute(
        DataCommand(
            mutations=[
    ...
    await cqrs_app.command_connection_manager.disconnect_all()
