...
async def test_schema_command(lakehouse_app: AsyncLakehouseApplication) -> None:
    ...
    connections = [
        await lakehouse_app.default_connection_manager.get_connection_pool(user_schema.name).get_connection(),
        await lakehouse_app.lakehouse_connection_manager.get_connection_pool(user_schema.name).get_connection(),
    ]
...
@pytest.mark.asyncio
async def test_data_command(lakehouse_app: AsyncLakehouseApplication) -> None:
    ...
    connections = [
        await lakehouse_app.default_connection_manager.get_connection_pool('customers').get_connection(),
        await lakehouse_app.lakehouse_connection_manager.get_connection_pool('customers').get_connection(),
    ]
...
@pytest.mark.asyncio
async def test_data_query(lakehouse_app: AsyncLakehouseApplication, mocker: MockerFixture) -> None:
    service = Container.services.get(AsyncDataCommandService)
    ...
    await lakehouse_app.lakehouse_connection_manager.disconnect_all()
