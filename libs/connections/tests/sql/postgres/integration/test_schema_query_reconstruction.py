"""Round-trip introspection tests for reconstructed PostgreSQL schema metadata.

These require a live PostgreSQL (provided by the integration conftest, which spins up a
container when no server is reachable). Each test creates a table/index exercising one
feature PROD previously dropped or rendered as a raw string, then asserts
``introspect_schema`` reconstructs it into the correct typed model.
"""
...
def test__check_constraint_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute(
        'CREATE TABLE products (id INT, price INT NOT NULL, CONSTRAINT chk_price CHECK (price > 0))'
    )

    schema = _introspect_one(database_connection, 'products')
...
def test__typed_default_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute(
        "CREATE TABLE accounts (id INT, balance INT NOT NULL DEFAULT 100, status TEXT NOT NULL DEFAULT 'active')"
    )

    schema = _introspect_one(database_connection, 'accounts')
...
def test__generated_column_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute(
    ...
    schema = _introspect_one(database_connection, 'rectangles')
...
def test__fk_referential_actions_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute('CREATE TABLE parents (id INT PRIMARY KEY)')
    database_connection.execute(
    ...
    schema = _introspect_one(database_connection, 'children')
...
def test__desc_index_direction_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute('CREATE TABLE events (id INT, ts INT NOT NULL)')
    database_connection.execute('CREATE INDEX idx_ts_desc ON events (ts DESC)')

    schema = _introspect_one(database_connection, 'events')
...
def test__include_columns_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute('CREATE TABLE covering (id INT, a INT, b INT)')
    database_connection.execute('CREATE INDEX idx_cover ON covering (a) INCLUDE (b)')

    schema = _introspect_one(database_connection, 'covering')
...
def test__exclusion_constraint_reconstructed(database_connection: PostgresConnection) -> None:
    database_connection.execute('CREATE EXTENSION IF NOT EXISTS btree_gist')
    database_connection.execute(
        'CREATE TABLE bookings (id INT, room_id INT, CONSTRAINT no_overlap EXCLUDE USING gist (room_id WITH =))'
    )

    schema = _introspect_one(database_connection, 'bookings')
