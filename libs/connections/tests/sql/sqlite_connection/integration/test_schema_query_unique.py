...
def test__without_inline_unique(database_connection: SqliteConnection) -> None:
    create_table_sql = "CREATE TABLE 'Fixture' ('partition_key' TEXT NOT NULL, 'class_name' TEXT, 'external_id' TEXT NOT NULL, 'data' JSONB NOT NULL, CONSTRAINT 'pk_fixture' PRIMARY KEY ('partition_key'), CONSTRAINT 'unq_fixture_external_id' UNIQUE ('external_id'))"
    database_connection.execute(create_table_sql)
