"""Round-trip introspection tests for reconstructed SQLite schema metadata (async)."""
...
async def test__check_constraint_reconstructed(database_connection: AsyncSqliteConnection) -> None:
    ...
    schema = await _get_schema(database_connection, 'products')
...
async def test__typed_default_reconstructed(database_connection: AsyncSqliteConnection) -> None:
    await database_connection.execute(
    ...
    schema = await _get_schema(database_connection, 'accounts')
...
async def test__generated_column_reconstructed(database_connection: AsyncSqliteConnection) -> None:
    await database_connection.execute(
    ...
    schema = await _get_schema(database_connection, 'rectangles')
    area = next(p for p in schema.properties if p.name == 'area')

    assert area.generated == Combined(_field_ref('width'), '*', _field_ref('height'))


async def test__partial_index_condition_reconstructed(database_connection: AsyncSqliteConnection) -> None:
    await database_connection.execute(
        'CREATE TABLE "tasks" ("id" INTEGER PRIMARY KEY, "done" INTEGER NOT NULL, "priority" INTEGER NOT NULL)'
    )
    await database_connection.execute('CREATE INDEX "idx_pending" ON "tasks" ("priority") WHERE done = 0')

    schema = await _get_schema(database_connection, 'tasks')
...
async def test__fk_referential_actions_reconstructed(database_connection: AsyncSqliteConnection) -> None:
    await database_connection.execute('CREATE TABLE "parents" ("id" INTEGER PRIMARY KEY)')
    await database_connection.execute(
    ...
    schema = await _get_schema(database_connection, 'children')
...
async def test__desc_index_direction_reconstructed(database_connection: AsyncSqliteConnection) -> None:
    await database_connection.execute('CREATE TABLE "events" ("id" INTEGER PRIMARY KEY, "ts" INTEGER NOT NULL)')
    await database_connection.execute('CREATE INDEX "idx_ts_desc" ON "events" ("ts" DESC)')

    schema = await _get_schema(database_connection, 'events')
